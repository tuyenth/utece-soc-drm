...
#include "MatlibSigProToolbox_FXP.cpp"
#include <sys/time.h>

#define WSU_FIXEDIII 1
//#define WSU_STANDALONE 1
...
CMatlibVector<CReal> Nuttallwin(const int iLen)
{
    ...
    const CFix rA0 = (CReal) 0.3635819;
	const CFix rA1 = (CReal) 0.4891775;
	const CFix rA2 = (CReal) 0.1365995;
	const CFix rA3 = (CReal) 0.0106411;

	const CFix rArg = (CFix) 2.0 * (CFix)crPi / (iLen - 1);
	CFix fvRetTmp;

	for (int i = 0; i < iLen; i++)
	{
		fvRetTmp = rA0 - rA1 * Cos(rArg * i) +
			rA2 * Cos(rArg * i * 2) - rA3 * Cos(rArg * i * 3);
		fvRet[i] = (CReal) fvRetTmp;
	}
...
#if WSU_STANDALONE
void wsu_fir(int iDecSizeY, int iDecFact, int iSizeFiltHist, 
	CMatlibVector<CComplex>& cvY, int iSizeB, 
	const CMatlibVector<CComplex>& cvB,
	CMatlibVector<CComplex>& cvXNew)
{
	int m, n, iCurPos;
	...
	for (m = 0; m < iDecSizeY; m++)
	{
		...
		cvY[m] = (CReal) 0.0;
		for (n = 0; n < iSizeB; n++) {
			FComplex f1(cvB[n].real(), cvB[n].imag());
			FComplex f2(cvXNew[iCurPos - n].real(), cvXNew[iCurPos - n].imag());
			fresult += f1*f2;
		}

		CComplex cvYmTmp((double)fresult.real(), (double)fresult.imag());
		cvY[m] = cvYmTmp;
#else
		iCurPos = m * iDecFact + iSizeFiltHist;
								   ...
								   const int iDecFact)
{
...
#if WSU_FIXEDIII
        iDecSizeY =
		    (int) (((CFix) iSizeXNew - iSizeFiltHist - 1) / iDecFact + 1);
#else
		iDecSizeY = 
...
#if WSU_STANDALONE
	wsu_fir(iDecSizeY, iDecFact, iSizeFiltHist, cvY, iSizeB, cvB, cvXNew);
#else
	/* FIR filter */
	for (m = 0; m < iDecSizeY; m++)
	{
	...
	printf("\nMFiltFlp,n=%d,t=%ld us",++numtimes,mtime );
