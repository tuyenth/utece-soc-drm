...
#define FTICONST_PDS_EST_TISYNC				((FXP) 0.25) /* sec */
...
//#define CONTR_SAMP_OFF_INT_FTI				FXP ((_REAL) 0.001)
#define CONTR_SAMP_OFF_INT_FTI				FXP ((_REAL) 0.001)
...
public:
	...
	void Process(CParameter& Parameter, CComplexVector& veccChanEst,
				 int iNewTiCorr, FXP& rLenPDS_fxp, FXP& rOffsPDS_fxp);
...
protected:
	...
	//CFComplexVector			veccPilots_fxp;
	int					iNumIntpFreqPil;
	CFftPlans				FftPlan;
	int					iScatPilFreqInt;
	int					iNumCarrier;
	CRealVector				vecrAvPoDeSp;
	//CFRealVector				vecrAvPoDeSp_fxp;
	CReal					rLamAvPDS;
	//CFReal				rLamAvPDS_fxp;
	...
	//CFRealVector				vecrAvPoDeSpRot_fxp;
	int					iSymDelay;
	CShiftRegister<int>			vecTiCorrHist;   //ok??
	CShiftRegister<int>			veciNewMeasHist;
	
	//CReal					rFracPartTiCor;
        CFReal					rFracPartTiCor_fxp;
	...
	CFReal					rEstPDSEnd_fxp; /* Estimated end of PSD, fixed */
	CFReal					rEstPDSBegin_fxp; /* Estimated beginning of PSD, fixed */
	

	CFReal					rFracPartContr_fxp;

	ETypeTiSyncTrac				TypeTiSyncTrac;

	CShiftRegister<long long>		veciSRTiCorrHist;
	int					iLenCorrectionHist;
	long long				iIntegTiCorrections;
	CReal					rSymBloSiIRDomain;
	int					iResOffsetAcquCnt;
	int					iResOffAcqCntMax;
	long long	      			iOldNonZeroDiff;

	CReal GetSamOffHz(long long iDiff, long long iLen);
};
