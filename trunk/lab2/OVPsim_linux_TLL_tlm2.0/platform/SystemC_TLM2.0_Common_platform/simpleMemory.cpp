...
/* derived from TLM examples memory.cpp by Imperas Ltd */


#include "reporting.h"                      // Reporting convenience macros
#include "simpleMemory.h"                   // Header for this class
...
{
/// Allocate and initialize an array for the target's memory
...
)
{
  ...
  switch (command)
  {
    default:
    {
      ...
      {
        msg << " Unsupported Command Extension";
...
)
{
  ...
  switch (command)
  {
    default:
    {
      if (m_previous_warning == false)
      {
        msg << " Unsupport GP command extension";
...
)
{
  ...
  else
  {
    ...
    {
      msg << " address " << address << " : " << address + length-1 << " will go out of bounds";
      REPORT_WARNING(filename, __FUNCTION__, msg.str());
