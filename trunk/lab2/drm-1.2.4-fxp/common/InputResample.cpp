...
void CInputResample::ProcessDataInternal(CParameter& ReceiverParam)
{
	...
	else
	{
		...
		/*iOutputBlockSize = ResampleObj.Resample(pvecInputData, pvecOutputData,
			...
			(SOUNDCRD_SAMPLE_RATE - rSamRateOffset)); CHANGE */

		iOutputBlockSize = ResampleObj.Resample(pvecInputData, pvecOutputData,
			(_REAL) SOUNDCRD_SAMPLE_RATE /
			(SOUNDCRD_SAMPLE_RATE - rSamRateOffset));

        /*printf ("\n Result (_REAL) = %f", (_REAL) SOUNDCRD_SAMPLE_RATE/ (SOUNDCRD_SAMPLE_RATE - rSamRateOffset));
        resultFX = (FXP) (SOUNDCRD_SAMPLE_RATE / (SOUNDCRD_SAMPLE_RATE - rSamRateOffset));
        printf ("\n FXP = %d %f %d", resultFX.GetValue(), resultFX.GetFValue(), resultFX.GetIValue());*/   
	}
}
