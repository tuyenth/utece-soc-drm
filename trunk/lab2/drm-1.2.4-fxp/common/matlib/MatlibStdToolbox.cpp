...
#include "MatlibStdToolbox_FXP.cpp"


template 
void CMatlibMatrix<FComplex>::Init(int iNRowLen, int iNColLen, FComplex tIniVal);
template 
void CMatlibMatrix<CComplex>::Init(int iNRowLen, int iNColLen, CComplex tIniVal);
...
CMatlibVector<int> Sort(const CMatlibVector<int>& rvI)
{
	const int iSize = rvI.GetSize();
	const int iEnd = iSize - 1;
	CMatlibVector<int> fvRet(iSize, VTY_TEMP);
...
/* This function is not listed in the header file. It shall be used only for
   Matlib internal calculations */
FComplex _integral(MATLIB_CALLBACK_QAUD f, const FXP a, const FXP b,
				   const FXP errorBound, FXP& integralBound,
				   _BOOLEAN& integralError, const FXP ru)
{
/*
	The following code (inclusive the actual Quad() function) is based on a
	JavaScript Example written by Lucio Tavernini. The code is hosted at
...
*/
	...
	FXP		left, right, h, h6, bound;
	FComplex	fa, fb, v1, v2, error, value;
	int			m1, jend, mstart, j;

	if (integralError)
	  return FXP(_MAXSHORT); /* NaN */

	/* Integrate over [a,b]. Initialize */
	const int max = 1024;
	FXPVector x(max);
	FComplexVector f1(max);
	FComplexVector f2(max);
	FComplexVector f3(max);
	FComplexVector v(max);
	...
	f2[0] = f((FXP) 0.5 * (a + b));
	f3[0] = f(b);
	v[0] = h * (f1[0] + (FXP) 4.0 * f2[0] + f3[0]) / (FXP) 6.0;
	...
	do
	{
		...
		do
		{
			...
			fa = f(x[j] + (FXP) 0.5 * h);
			fb = f(x[j] + (FXP) 1.5 * h);
			v1 = h6 * (f1[j] + (FXP) 4.0 * fa + f2[j]);
			v2 = h6 * (f2[j] + (FXP) 4.0 * fb + f3[j]);
			error = (v[j] - v1 - v2) / (FXP) 15.0;
...
FComplex Quad(MATLIB_CALLBACK_QAUD f, const FXP a, const FXP b,
			  const FXP errorBound)
{
	/* Set globals */
	/* Generate rounding unit */
	FXP value;
