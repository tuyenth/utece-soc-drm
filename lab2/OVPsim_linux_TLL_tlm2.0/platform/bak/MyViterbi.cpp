...
void MyViterbi::initiatorTransport(transaction_type& trans, sc_core::sc_time& t)
{
	...
	switch( trans.get_command() ) 
	{
        case TLM_READ_COMMAND :  
			what = "read";
			if (addr == 0xd3000004) // Return status
			{
				//intr.write(0); // This would disable the interrupt line again
				data = this->status;
				memcpy(dataPtr, &data, len); 
			}
			else if (addr == 0xd300000C) //# of bytes to ARM
			{
				data = this->nDeOutput;
				memcpy(dataPtr, &data, len); 
				pOutput = vecOutputBits;
			}
			else if (addr >= 0xd3000300 && addr <= 0xd30003FC )
			{
				data = *(this->pOutput);
				this->pOutput++;
				if(this->pOutput - vecOutputBits > this->nDeOutput>>2)
					cout << "Output Pointer Out of Boundary" << endl;
			}
			break;
        case TLM_WRITE_COMMAND:  
			what = "write" ;  
			if (addr == 0xd3000000) // Command
			{
				if(data == 0) { //shutdown
			...
			else if(addr == 0xd3000008) // # of bytes from ARM
			{
				nDeInput = data;
				pInput = vecNewDistance;
			}
			else if(addr == 0xd3000010) // The following 9 addresses are for init input
				this->eNewCodingScheme = data;
			else if(addr == 0xd3000014)	
				this->eNewChannelType = data;
			else if(addr == 0xd3000018)
				this->iN1 = data;
			else if(addr == 0xd300001C)
				this->iN2 = data;
			else if(addr == 0xd3000020)
				this->iNewNumOutBitsPartA = data;
			else if(addr == 0xd3000024)
				this->iNewNumOutBitsPartB = data; 
			else if(addr == 0xd3000028)
				this->iPunctPatPartA = data;
			else if(addr == 0xd300002C)
				this->iPunctPatPartB = data;
			else if(addr == 0xd3000030)	
				this->iLevel = data;
			else if(addr >= 0xd3000200 && addr <= 0xd30002FC)
			{
				*(this->pInput) = data;
				this->pInput++;
				nDeInput -= 4;
