...
int CResample::Resample(CVector<_REAL>* prInput, CVector<_REAL>* prOutput, _REAL rRation)
{
	/* Move old data from the end to the history part of the buffer and
	   add new data (shift register) */
        CVector<FXP> prIn(prInput->Size());
        for (int i=0; i<prIn.Size(); i++) prIn[i] = (*prInput)[i];
        vecrIntBuff_fxp.AddEnd(prIn, iInputBlockSize);
...
void CResample::Init(const int iNewInputBlockSize)
{
	...
	vecrIntBuff_fxp.Init(iInputBlockSize + iHistorySize, (_REAL) 0.0);
