...
* CVector base class                                                           *
\******************************************************************************/
template<class TData> class CVector : public vector<TData, allocator<TData> >
...
public:
	...
	CVector(const CVector<TData>& vecI) :
		vector<TData, allocator<TData> >(static_cast<const vector<TData, allocator<TData> >&>(vecI)), 
...
#endif
		vector<TData, allocator<TData> >::operator=(vecI);
...
protected:
	typename vector<TData, allocator<TData> >::iterator	pData;
												...
												const int iNewColumn)
{
	...
	if (iRow > 0)
	{
		...
		pData = new CVector<TData>[iRow];
		for (int i = 0; i < iRow; i++)
			pData[i].Init(iNewColumn);
	}
}
